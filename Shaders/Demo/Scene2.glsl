...
void main()
{
	...
	Cloud targetCloud = initCloud(1111111111, vec3(0, -11020, 0));
	targetCloud.baseColor = vec3(1, .7, .2);

	color += render( Ray( camP, camDir ), targetCloud);

	const float startBlendTime = 6;

	if(iGlobalTime > startBlendTime)
	{
		color += (iGlobalTime - startBlendTime)  * vec3(1, 1, 1);
	}
