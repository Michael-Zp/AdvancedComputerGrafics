...
float distScene(vec3 point)
{
	...
	vec3 planePoint = point;
	planePoint.x += sin(point.x - iGlobalTime * 10) * .1;
	planePoint.y += cos(point.y - iGlobalTime * 10) * .1;
	
	float distPlane = sPlane(planePoint, vec3(0.0, 1.0, 0.0), -0.5);
	point = opCoordinateRepetition(planePoint, vec3(1.0, 1.0, 1.0));
...
void main()
{
	...
	color = mix(color, vec3(0.2, 0.6, 1), factor); //line ii
