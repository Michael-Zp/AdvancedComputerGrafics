...
#define WEAKLIGHTCOLOR vec3(16.86, 10.76, 8.2) * 0.2
...
vec3 magicRandom = vec3(43758.5453123, 22578.1459123, 19642.3490423);
//vec3 magicRandom = vec3(iGlobalTime);

float hash1()
{
    return fract(sin(seed += 0.1) * magicRandom.x);
    //return fract(sin(seed) * magicRandom.x);
}

vec2 hash2()
{
    return fract(sin(vec2(seed+=0.1,seed+=0.1)) * magicRandom.xy);
    //return fract(sin(vec2(seed,seed)) * magicRandom.xy);
}

vec3 hash3()
{
    return fract(sin(vec3(seed+=0.1,seed+=0.1,seed+=0.1)) * magicRandom);
    //return fract(sin(vec3(seed,seed,seed)) * magicRandom);
}
...
float intersect( const vec3 ro, const vec3 rd, inout vec3 normal, out float material ) 
{
	...
	t = iPlane( ro, rd, vec4( 0.0, 1.0, 0.0,1.0 ) ); update(t, 1., vec3( 0., 1., 0.), res, normal);
	t = iPlane( ro, rd, vec4( 0.0, 0.0,-1.0,10.0 ) ); update(t, 5., vec3( 0., 0.,-1.), res, normal);
    t = iPlane( ro, rd, vec4( 1.0, 0.0, 0.0,1.0 ) ); update(t, 2., vec3( 1., 0., 0.), res, normal);
    t = iPlane( ro, rd, vec4( 0.0,-1.0, 0.0,6.49) ); update(t, 1., vec3( 0., -1., 0.), res, normal);
    t = iPlane( ro, rd, vec4(-1.0, 0.0, 0.0,6.59) ); update(t, 3., vec3(-1.,  0., 0.), res, normal);
...
vec3 traceEyePath( in vec3 ro, in vec3 rd ) 
{
    ...
    for(int i = 0; i < EYEPATHLENGTH; ++i) 
	{
		...
		{	//hit light source
            return color * matColor(material);
        }
...
void main()
{
    ...
    for( int i = 0; i < SAMPLES; ++i ) // loop over samples
	{
        ...
        color += traceEyePath( cameraPos, rd ) * dot(normalize(camera * vec3(p, 3.0)), normalize(vec3(rayDelta, 3.0)));
