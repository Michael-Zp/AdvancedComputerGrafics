...
float intersect( const vec3 ro, const vec3 rd, inout vec3 normal, out float material ) 
{
    ...
    t = iPlane( ro, rd, vec4(-1.0, 0.0, 0.0,5.59) ); update(t, 11., vec3(-1.,  0., 0.), res, normal);
...
bool matIsGlossy( const float mat ) 
{
	return mat > 10.5;
}
...
vec3 cosWeightedRandomHemisphereDirectionInCone(float degree) 
{
  	vec2 rnd = hash2(); //range [0,1]²

	//The common way to generate a cosine weighted hemisphere sampler is to generate uniform points on a disk, and then project them up to the hemisphere.
	float r = sqrt(rnd.x);
	float theta = degree * rnd.y;
	float z = sqrt(1.0 - rnd.x);
...
vec3 getBRDFRay( in vec3 n, const vec3 dirIn, const float material ) 
{
	...
	else if(matIsGlossy(material)) 
	{
		float degree = 25;
		return coordinateSystem( n ) * cosWeightedRandomHemisphereDirectionInCone(degree / 180 * PI);
	}
	else
	{
